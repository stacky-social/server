...
class Rack::Attack
  ...
  Rack::Attack.safelist('allow from localhost') do |req|
    req.remote_ip == '127.0.0.1' || req.remote_ip == '::1' || req.remote_ip == '104.236.200.19'
  end
  ...
  throttle('throttle_authenticated_api', limit: 6_000, period: 5.minutes) do |req|
    req.authenticated_user_id if req.api_request?
  end

  throttle('throttle_per_token_api', limit: 6_000, period: 5.minutes) do |req|
    req.authenticated_token_id if req.api_request?
  end

  throttle('throttle_unauthenticated_api', limit: 6_000, period: 5.minutes) do |req|
