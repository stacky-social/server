...
class ReportService < BaseService
  ...
  def forward_to_origin!
    ...
    ActivityPub::DeliveryWorker.perform_async(payload, some_local_account.id, @target_account.inbox_url) unless @target_account.internal?
  end

  def forward_to_replied_to!
    # Send report to servers to which the account was replying to, so they also have a chance to act
    inbox_urls = Account.remote.where(domain: forward_to_domains).where(id: Status.where(id: reported_status_ids).where.not(in_reply_to_account_id: nil).where(ext_flag: nil).select(:in_reply_to_account_id)).inboxes - [@target_account.inbox_url, @target_account.shared_inbox_url] # NOTE: skip internal posts
